...
export function ProjectCard({ project, titleAs }: { project: ProjectItemType, titleAs?: keyof React.JSX.IntrinsicElements }) {
  ...
  // Projects with slug navigate to detail page; others use modal
  ...
  if (hasDetailPage) {
    return (
      <li className='group relative flex flex-col items-start h-full'>
        <Link
          href={detailHref}
          className="relative flex flex-col justify-between h-full w-full p-4 rounded-2xl border border-muted-foreground/20 shadow-sm transition-all group-hover:scale-[1.03] group-hover:shadow-md group-hover:bg-muted/5 text-left"
